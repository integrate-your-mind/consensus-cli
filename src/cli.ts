...
import { runHarnesses } from "./cli/harnesses.js";
...
function runOneShot(
  ...
  action()
    ...
    .catch(async (error) => {
      process.stderr.write(`[consensus] ${label} error: ${String(error)}\n`);
      await disposeObservability().catch(() => undefined);
      process.exit(1);
    });
}
...
if (args[0] === "setup") {
  runOneShot("setup", runSetup);
} else if (args[0] === "graph") {
  runOneShot("graph", () => runGraph(args.slice(1)));
} else if (args[0] === "harnesses") {
  runOneShot("harnesses", () => runHarnesses(args.slice(1)));
} else {
  ...
  function printHelp(): void {
    ...
    process.stdout.write(
      "  setup                Configure Codex notify hook (recommended first step)\n"
    );
    process.stdout.write(
      "  graph                Inspect agent transitions and detect loops\n"
    );
    process.stdout.write(
      "  harnesses            List harnesses, evidence tiers, and setup guidance\n"
    );
  ...
  runPromise(instrumented)
    ...
    .catch(async (error) => {
      process.stderr.write(`[consensus] cli error: ${String(error)}\n`);
      await disposeObservability().catch(() => undefined);
      process.exit(1);
    });
}
