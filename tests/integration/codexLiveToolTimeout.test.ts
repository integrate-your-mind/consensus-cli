...
liveTest(
  ...
  async (t) => {
    ...
    try {
      ...
      const baseline = await tmux.capture();
      const launchCommand = await startCodexInteractiveInTmux({
        session: tmux,
      ...
      const ready = await waitForCodexReady(tmux, {
        timeoutMs: 15_000,
        baseline,
        launchCommand,
      });
      if (ready.status === "auth") {
        t.skip(`codex auth required in this environment.${formatPaneExcerpt(ready.output)}`);
        return;
      }
      if (ready.status !== "ready") {
        t.skip(`codex did not become ready (${ready.status}).${formatPaneExcerpt(ready.output)}`);
        return;
      }
      ...
      const paneOutput = await waitForPaneOutput(
        tmux,
        (output) => output.includes(testTag),
        10_000
      );
      if (/unauthorized|login|api key|missing bearer/i.test(paneOutput)) {
        t.skip(`codex auth required in this environment.${formatPaneExcerpt(paneOutput)}`);
        return;
      }
      ...
      try {
        ...
        t.skip(`codex did not start session or pid: ${String(err)}${excerpt}`);
        return;
      }
