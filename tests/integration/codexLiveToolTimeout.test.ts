...
async function waitForAgent(
  ...
  tag: string,
  timeoutMs: number,
  tmux?: Awaited<ReturnType<typeof startTmuxSession>>
  ...
  throw new Error(
    `Timed out waiting for codex agent in snapshot. Agents: ${JSON.stringify(summary)}${suffix}`
  );
}
...
liveTest(
  ...
  async (t) => {
    ...
    try {
      ...
      const agent = await waitForAgent(port, sessionPath, project.root, pid, testTag, 30_000, tmux);
