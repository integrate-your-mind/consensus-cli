...
export type CodexReadyStatus = "ready" | "auth" | "error";

export type CodexReadyResult = {
  status: CodexReadyStatus;
  output: string;
};

const CODEX_AUTH_PATTERN = /unauthorized|login|api key|missing bearer/i;
const CODEX_ERROR_PATTERN =
  /command not found|no such file or directory|permission denied|exec format error|not a tty|panic|fatal/i;
const CODEX_READY_PATTERN =
  /\bOpenAI\b.*\bCodex\b|\bcodex[^\n]*[>#]|\bCodex CLI\b|\bType\b.*\bhelp\b|\bRun\s+\w+:\s*/i;

export function formatPaneExcerpt(output: string, maxChars = 2000): string {
...
export async function waitForPaneOutput(
  ...
  throw new Error(`Timed out waiting for tmux output${suffix}`);
}
...
export async function waitForCodexReady(
  session: TmuxSession,
  timeoutMs: number
): Promise<CodexReadyResult> {
  const start = Date.now();
  let lastOutput = "";
  while (Date.now() - start < timeoutMs) {
    const output = await session.capture();
    lastOutput = output;
    if (CODEX_AUTH_PATTERN.test(output)) return { status: "auth", output };
    if (CODEX_ERROR_PATTERN.test(output)) return { status: "error", output };
    if (CODEX_READY_PATTERN.test(output)) return { status: "ready", output };
    await delay(200);
  }
  const excerpt = formatPaneExcerpt(lastOutput);
  const suffix = excerpt ? `\nTmux output:\n${excerpt}` : "";
  throw new Error(`Timed out waiting for codex readiness${suffix}`);
}
...
function parseProcessList(output: string): ProcessInfo[] {
  ...
  for (const line of lines) {
    ...
    processes.push({ pid, ppid, startedAt, command });
  }
  return processes;
}

function buildChildIndex(processes: ProcessInfo[]): Map<number, number[]> {
  const index = new Map<number, number[]>();
  for (const proc of processes) {
    const list = index.get(proc.ppid);
    if (list) {
      list.push(proc.pid);
    } else {
      index.set(proc.ppid, [proc.pid]);
    }
  }
  return index;
}

function collectDescendants(rootPid: number, index: Map<number, number[]>): Set<number> {
...
function scoreCodexProcess(command: string): number {
  let score = 0;
  if (hasCodexToken(command)) score += 10;
  if (hasCodexVendorPath(command)) score += 6;
  const firstToken = command.split(/\s+/)[0] ?? "";
  if (isCodexBinary(firstToken)) score += 8;
  if (/codex/i.test(command)) score += 2;
  return score;
}
...
export async function waitForCodexPid(options: {
  ...
  const start = Date.now();
  const startCutoff = options.startAfterMs - 2000;
  while (Date.now() - start < options.timeoutMs) {
    ...
    const processes = parseProcessList(stdout.toString());
    const childIndex = buildChildIndex(processes);
    let candidates = processes.filter(
      (proc) => proc.startedAt && proc.startedAt >= startCutoff
    );
    if (options.panePid) {
      const descendants = collectDescendants(options.panePid, childIndex);
      candidates = candidates.filter((proc) => descendants.has(proc.pid));
    }
    const scored = candidates
      .map((proc) => ({ proc, score: scoreCodexProcess(proc.command) }))
      .filter((entry) => entry.score > 0)
      .sort((a, b) => {
        if (a.score !== b.score) return b.score - a.score;
        return (b.proc.startedAt ?? 0) - (a.proc.startedAt ?? 0);
      });
    if (scored.length > 0) return scored[0].proc.pid;
    if (options.panePid && candidates.length > 0) {
