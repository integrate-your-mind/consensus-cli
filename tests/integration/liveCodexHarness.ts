...
export async function waitForPaneOutput(
  ...
  const output = await session.capture();
  throw new Error(`Timed out waiting for tmux output.${formatPaneExcerpt(output)}`);
}
...
function extractBaseCommand(command: string): string {
  ...
  if (tokens[0] === "env") {
    for (let i = 1; i < tokens.length; i += 1) {
      if (!tokens[i].includes("=")) return path.basename(tokens[i]);
    }
  }
  return path.basename(tokens[0]);
}

function stripCommandEcho(output: string, launchCommand?: string): string {
  const tokens = [
    "CODEX_HOME=",
  ...
  const filtered = lines.filter((line) => {
    if (launchCommand && line.includes(launchCommand)) return false;
...
function regexFromEnv(envKey: string, fallback: RegExp): RegExp {
...
export function formatPaneExcerpt(output: string, maxChars = 1200): string {
  const trimmed = output.trim();
  if (!trimmed) return "\n--- tmux pane (empty) ---";
  const excerpt = trimmed.length > maxChars ? trimmed.slice(-maxChars) : trimmed;
  return `\n--- tmux pane (tail) ---\n${excerpt}`;
}
...
export async function capturePaneExcerpt(
  ...
  const output = await session.capture();
  return formatPaneExcerpt(output, maxChars);
}
...
): Promise<{ status: "ready" | "auth" | "timeout"; output: string }> {
  const readyRegex =
    options.readyRegex ??
    regexFromEnv("CODEX_READY_REGEX", /(^|\n)\s*[^\n]{0,120}[>#]\s*$/m);
  const authRegex =
    options.authRegex ??
    regexFromEnv(
      "CODEX_AUTH_REGEX",
      /unauthorized|login|api key|missing bearer|missing token|sign in|signin|log in/i
    );
  const baseline = normalizeOutput(
    stripCommandEcho(options.baseline ?? "", options.launchCommand)
  ).trim();
  const start = Date.now();
  while (Date.now() - start < options.timeoutMs) {
    const output = await session.capture();
    const cleaned = normalizeOutput(stripCommandEcho(output, options.launchCommand)).trim();
    if (cleaned && cleaned !== baseline) {
      if (authRegex.test(cleaned)) return { status: "auth", output };
      if (readyRegex.test(cleaned)) return { status: "ready", output };
...
export async function waitForCodexPid(options: {
  ...
  const codexBase = options.codexBin ? path.basename(options.codexBin) : undefined;
  const codexBaseRegex = codexBase
    ? new RegExp(`(?:^|\\s|[\\/])${escapeRegex(codexBase)}(?:\\s|$)`, "i")
    : undefined;
  const ignoredBases = new Set([
    "bash",
    "sh",
    "zsh",
    "fish",
  ...
  while (Date.now() - start < options.timeoutMs) {
    ...
    for (const line of lines) {
      ...
      if (!startedAt || startedAt < options.startAfterMs - 2000) continue;
      ...
      processes.push({
        ...
        startedAt,
        cmd,
        base: extractBaseCommand(cmd).toLowerCase(),
      });
    }
    const byPid = new Map(processes.map((proc) => [proc.pid, proc]));
    const isDescendantOf = (pid: number, ancestorPid: number): boolean => {
      let current = pid;
      for (let i = 0; i < 60; i += 1) {
        const proc = byPid.get(current);
        if (!proc) return false;
        if (proc.ppid === ancestorPid) return true;
        if (proc.ppid === 0 || proc.ppid === current) return false;
        current = proc.ppid;
      }
      return false;
    };
    let codexCandidates = processes.filter((proc) => {
      if (hasCodexToken(proc.cmd)) return true;
      if (codexBaseRegex && codexBaseRegex.test(proc.cmd)) return true;
      return false;
    });
    if (options.panePid) {
      const paneMatches = codexCandidates.filter((proc) =>
        isDescendantOf(proc.pid, options.panePid!)
      );
      if (paneMatches.length > 0) codexCandidates = paneMatches;
    }
    if (codexCandidates.length > 0) {
      codexCandidates.sort((a, b) => b.startedAt - a.startedAt);
      return codexCandidates[0].pid;
    }
    if (options.panePid) {
      let descendants = processes.filter((proc) => isDescendantOf(proc.pid, options.panePid!));
      let filtered = descendants.filter((proc) => !ignoredBases.has(proc.base));
      if (filtered.length === 0) filtered = descendants;
  ...
  throw new Error("Timed out waiting for codex pid");
}
